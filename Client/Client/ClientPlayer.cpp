...
void Player::sendInputToServer(SOCKET s) {
    ...
    // 입력 패킷을 서버에 전송
    int retval = send(s, (char*)&inputPacket, sizeof(inputPacket), 0);
    if (retval == SOCKET_ERROR) {
        int errorCode = WSAGetLastError();
        std::wstring errorMessage = L"Failed to send input (Player). Error code: " + std::to_wstring(errorCode);
        MessageBox(NULL, errorMessage.c_str(), L"Send Error", MB_ICONERROR | MB_OK);
    }
