...
GameFramework::GameFramework()
    ...
    player = new Player(mapWidth / 2.0f - 100, mapHeight / 2.0f, 2.0f, 0.2f, this); // gameFramework 포인터 전달
    player2 = new Player(mapWidth / 2.0f, mapHeight / 2.0f, 2.0f, 0.2f, this); // gameFramework 포인터 전달
    player3 = new Player(mapWidth / 2.0f + 100, mapHeight / 2.0f, 2.0f, 0.2f, this); // gameFramework 포인터 전달

    players.push_back(player); // players 벡터에 player 추가
    players.push_back(player2); // players 벡터에 player 추가
    players.push_back(player3); // players 벡터에 player 추가

    // xPos, yPos, speed, animationSpeed, gameframeworkPtr
    player->SetBounds(mapWidth, mapHeight);
    player2->SetBounds(mapWidth, mapHeight);
    player3->SetBounds(mapWidth, mapHeight);
...
void GameFramework::Draw(HDC hdc) {
    ...
    for (Player* p : players)
    {
        ...
        string str = "     CURRENT RENDERING PLAYER ID: " + to_string(p->ID) + "\n";
        //string str = to_string(players.size()) + "\n";
...
void GameFramework::receiveGameData(SOCKET s)
{
    ...
    UpdateObstacleInfo(recv_obstacles);
...
void GameFramework::UpdatePlayerInfo(vector<PlayerStatusPacket> packet)
{
    for (int i = 0; i < 3; ++i)
    {
...
void GameFramework::UpdateEnemyInfo(vector<s_enemyPacket> packet)
{
    int size = packet.size();
    enemies.resize(size);
    for (int i = 0; i < size; ++i)
    {
        enemies[i]->setX(packet[i].s_enemyPosX);
        enemies[i]->setY(packet[i].s_enemyPosY);
        enemies[i]->setHealth(packet[i].s_enemyHealth);
    }
}

void GameFramework::UpdateItemInfo(vector<s_itemPacket> packet)
{
    int size = packet.size();
    items.resize(size);
    for (int i = 0; i < size; ++i)
    {
        items[i]->setX(packet[i].s_itemPosX);
        items[i]->setY(packet[i].s_itemPosY);
    }
}

void GameFramework::UpdateObstacleInfo(vector<s_obstaclePacket> packet)
{
    int size = packet.size();
    obstacles.resize(size);
    for (int i = 0; i < size; ++i)
    {
        obstacles[i]->SetX(packet[i].s_obstaclePosX);
        obstacles[i]->SetY(packet[i].s_obstaclePosY);
    }
}

void GameFramework::UpdateBulletInfo(vector<s_bulletPacket> packet)
{
    int size = packet.size();
    bullets.resize(size);
    for (int i = 0; i < size; ++i)
    {
        bullets[i]->x = packet[i].s_bulletPosX;
        bullets[i]->y = packet[i].s_bulletPosY;
    }
}
