...
void Bullet::Update(float frameTime) { // 서버
    if (isHit) {
...
void Bullet::Draw(HDC hdc, float offsetX, float offsetY) {//클라
    if (isHit) {
...
void Bullet::DrawHitEffect(HDC hdc, float offsetX, float offsetY) {//클라
...
bool Bullet::isEffectFinished() const {//클라
